...
- Auto-detect usable FFmpeg H.264 hardware encoders, with manual encoder selection and CPU fallback
...
Pre-built Windows builds are available on the [Releases](https://github.com/EternalAcacia/SLVGenerator/releases) page. Download the latest `SLVGenerator-v1.2.0-windows.zip`, extract it, and run `SLVGenerator.exe`.
...
- 渲染前生成首帧预览
- 在后台线程渲染 MP4，并显示进度更新
- 自动检测可用的 FFmpeg H.264 硬件编码器，支持手动选择编码器和 CPU 回退
...
Windows 预编译版本可在 [Releases](https://github.com/EternalAcacia/SLVGenerator/releases) 页面下载。下载最新的 `SLVGenerator-v1.2.0-windows.zip`，解压后运行 `SLVGenerator.exe` 即可。
